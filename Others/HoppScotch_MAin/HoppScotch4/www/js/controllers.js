...
.controller('PlaceCtrl', function($rootScope, $ionicHistory, $scope, $state, $timeout, $ionicPopup, $http,$ionicLoading, $ionicModal, geo, place, searchResults) {
  ...
  $scope.getMessage = function () {
    ...
    $http({
          method: "post",
          url: 'http://104.236.134.190:3000/messages/',
  ...
  $scope.postMessage = function () {
      ...
      if (message.length > 144) {
        ...
        $http({
              method: "post",
              url: "http://104.236.134.190:3000/messages/newMessage",
              headers: {'Content-Type': 'application/json'},
              data: {
                place_id: $scope.place.place_id,
                message: safeMessage
  ...
  $scope.up = function(message) {
    ...
    $http({
      method: "post",
      url: "http://104.236.134.190:3000/messages/vote",
      headers: {'Content-Type': 'application/json'},
      data: {
        _id: message._id,
        vote: 1
...
.controller('PlacesCtrl', function($rootScope, $scope, $state, $timeout, $ionicPopup, $ionicModal,$http, $ionicLoading, searchResults, place, geo) {
  ...
  $scope.closeModal = function() {
    ...
    $http({
          method: "post",
          url: "http://104.236.134.190:3000/places/updateRating",
          headers: {'Content-Type': 'application/json'},
          data: {
            _id:    $scope.place._id,
  ...
  $scope.goToPlace = function (place) {
    ...
    */
  //if ($scope.geo.latitude >= latLeft && $scope.geo.latitude <= latRight && $scope.geo.longitude<=longBottom && $scope.geo.longitude>= longTop) { // real phone
  ...
  //  if (1) { // allow to checkin from anywhere for map testing.
      ...
      if (!$scope.place._id) {
        ...
        $http({
            method: "post",
            url: "http://104.236.134.190:3000/places/newPlace",
            headers: {'Content-Type': 'application/json'},
            data: {
        ...
        $http({
            method: "post",
            url: "http://104.236.134.190:3000/places/checkin",
            headers: {'Content-Type': 'application/json'},
            data: {
              place: $scope.place.place_id
            }
...
.controller('MapCtrl', function($rootScope, initialGeo, $state, $ionicPopup, $scope, $stateParams, $timeout, searchResults,$http, geo, $cordovaGeolocation, $ionicLoading) {
  var map;
  console.log("Initial Geo is : ", JSON.stringify(initialGeo));
  //console.log("original geo" + JSON.stringify(geo));
  $scope.searchResults = searchResults;
  $scope.geo = geo;
  $rootScope.onMap = true;

  if (!$rootScope.clickedSomewhere) {
    $scope.geo.lat = initialGeo.coords.latitude;
    $scope.geo.long = initialGeo.coords.longitude;

    $rootScope.actualLat = initialGeo.coords.latitude;
    $rootScope.actualLong = initialGeo.coords.longitude;
    console.log('from before clicked somewhere' + JSON.stringify(geo));
  }
      ...
      buttons: [
          ...
          } },
        {
          ...
          onTap: function(e) {
            $http({
                method: "post",
                url: "http://104.236.134.190:3000/person/notNew",
                headers: {'Content-Type': 'application/json'},
                data: {
                  uuid: $rootScope.uuid
                }
  ...
  $scope.init = function (){
    ...
    // //set new center
    // resetCenter_interval=3000;
    // var resetCenter = function(map, mapOptions)
    // {
    //   //Execution code
    ...
    //     map = new google.maps.Map(document.getElementById("map"), mapOptions);
    //   }, resetCenter_interval);
    // };
    // resetCenter(map, mapOptions);
...
.controller('LearnCtrl', function($scope,$ionicSlideBoxDelegate, $state, $timeout, geo, $ionicLoading) {
